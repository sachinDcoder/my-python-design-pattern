...
class Relationships(RelationshipBrowser):
    ...
    def find_all_children_of(self, name):
        return list(filter(lambda relation: relation[0].name == name and relation[1] == Relationship.PARENT, self.relations))


class Research:
    def __init__(self, browser, person):
        for p in browser.find_all_children_of(person.name):
            print(f'{person.name} has a child called {p[2].name}')
